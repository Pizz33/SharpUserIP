...
namespace SharpUserIP
{
    internal class GetLog
    {
        ...
        public static List<string> GetDC(string dc)
        {
            List<string> list = new List<string>();
            string DomainName = dc ?? Environment.GetEnvironmentVariable("USERDNSDOMAIN");
            DirectoryEntry directoryEntry = new DirectoryEntry($"LDAP://{DomainName}:389", null, null, AuthenticationTypes.Secure);
        ...
        public static void QueryLog(int days, bool all, string dc = null, string user = null, string outFile = null)
        {
            try
            {
                ...
                if (all)
                {
                    foreach (string dcs in GetDC(dc))
                    {
